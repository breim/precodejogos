...
task :add_jogos => :environment do
	...
	Crawler.all.each do |c|
		...
		c.site_paginas.times do |pagina|
			...
			agent = Mechanize.new.get(c.site_link + pagina.to_s) if c.status == 2			

			total = agent.search(c.product_title).count
			puts total
			total.times do |t|
				productPrice =	agent.search(c.product_price).map(&:text)[t].gsub(/[^0-9,]/, '').gsub(",",".") if agent.search(c.product_price).map(&:text)[t].present?
				productTitle =	agent.search(c.product_title).map(&:text)[t]
				...
				elsif verificar == nil && productPrice != nil
